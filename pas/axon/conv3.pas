...
begin
 ...
 delete(inndata,1,pos('U:',inndata)+2);
 val(GetString,u1,dum);
 delete(inndata,1,pos('V:',inndata)+2);
 val(GetString,v1,dum);
 ...
 Points^[nv,2] := d3;
 TexCoords^[nv,0] := round(u1*TexSize);
 TexCoords^[nv,1] := round(v1*TexSize);
 inc(nv);
end;
...
begin
 ...
 begin
  writeln;
  writeln ('Usage: CONVERT infile FN PN En Lm LabelNum TN');
  Writeln;
 ...
 if paramstr(7) = '1' then IncludeTextureCoords := true;
