...
Procedure GetFilename (x1,y1:word;title,mask:string;var fname:string);
Procedure MessageBox(x1,y1:word;s,title:string);
...
begin
 GetDir(0,curpath);
 InitDialog(d,x1,y1,x1+157,y1+102,title,1);
 AddInputField(d,0,0,15,'',NOP,1);

 AddFrame(d,0,10,86,66,SelectFile,2);
 AddVSlider(d,88,10,98,66,0,64,DrawList,3);

 AddText(d,112,10,'Mask',NOP,4);
 AddInputField(d,102,17,6,'*.*',NewMask,5);

 AddButton(d,102,37,147,46,'OK',NOP,99);
 AddButton(d,102,47,147,56,'CH.DIR',NOP,98);
 AddButton(d,102,57,147,66,'Cancel',NOP,100);
