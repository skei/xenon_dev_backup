...
static void CompElementDeriv (float pp,  float p,   float pn,
 ...
 // compute derivative for key "key".
static void CompDeriv (sKeyframe *keyp, sKeyframe *key, sKeyframe *keyn)
 {
 ...
 // compute derivative for first key "key".
static void CompDerivFirst (sKeyframe *key, sKeyframe *keyn, sKeyframe *keynn)
 {
 ...
 // compute derivative for last "key".
static void CompDerivLast (sKeyframe *keypp, sKeyframe *keyp, sKeyframe *key)
 {
  float f20, f10, v20, v10;
 ...
 // compute derivative for first key "key".
static void CompDerivLoopFirst (sKeyframe *keyp, sKeyframe *key, sKeyframe *keyn, float lf)
 {
  float tm, cm, cp, bm, bp, tmcm, tmcp;
  float ksm, ksp, kdm, kdp;
  float dt, fp, fn, c;
 ...
 // compute derivative for last key "key".
static void CompDerivLoopLast (sKeyframe *keyp, sKeyframe *key, sKeyframe *keyn, float lf)
 {
  float tm, cm, cp, bm, bp, tmcm, tmcp;
  float ksm, ksp, kdm, kdp;
  float dt, fp, fn, c;
 ...
 // compute derivative for keys "key" (2-key spline).
static void CompDerivTwo (sKeyframe *key)
 {
 ...
 // Compute derivatives for rotation keys.
static void CompAB (sKeyframe *prev, sKeyframe *cur, sKeyframe *next)
 {
  ...
  if (prev != NULL)
  {
   ...
   {
    Quat_Copy (&qprev,&QA);
    if (Quat_DotUnit (&QB,&qprev) < 0.0) Quat_Negate (&qprev, &qprev);
  ...
  if (next != NULL)
  {
   ...
   {
    Quat_Copy(&qnext,&QC);
    if (Quat_DotUnit(&QB,&qnext) < 0.0) Quat_Negate(&qnext, &qnext);
 ...
 // remap parameter between two keys to apply eases.
  ...
  else
  {
   ...
   else
   {
    ...
    return (1.0-(k/b)*t*t);
                                                   ...
                                                   // FIX LOOP FLAG!!!!!!
 // compute spline derivations.
int Spline_Init (sTrack *track)
 {
  sKeyframe *curr;
  sKeyframe *keys = track->Keys;
  sKeyframe *last = track->LastKey;
  ...
  if (keys->Next->Next)
  { // 3 or more keys
   for (curr = keys->Next; curr->Next; curr = curr->Next) CompDeriv (curr->Prev, curr, curr->Next);
   if (track->Flags & 0xffffffff/*clax_track_loop*/)
   {
...
int Spline_GetKey_Float (sTrack *track, float frame, float *out)
 {
  ...
  h[2] = t3 - 2 * t2 + t;
  h[3] = t3 - t2;
  *out =   (h[0]*keys->Val._float)
         + (h[1]*keys->Next->Val._float)
         + (h[2]*keys->dda)
         + (h[3]*keys->Next->dsa);
  return 0;//clax_err_ok;
 }
...
int Spline_GetKey_Vector (sTrack *track, float frame, sVector *out)
 {
  sKeyframe *keys;
  float  t, t2, t3;
  float  h[4];
  ...
  if (!keys->Next || frame < keys->Frame)
  { // frame is above last key
   Vector_Copy (out,&keys->Val._vector);
   return 0;//clax_err_ok;
  }
  t = (frame - keys->Frame) / (keys->Next->Frame - keys->Frame);
  t = spline_ease (t, keys->EaseFrom, keys->Next->EaseTo);
...
int Spline_GetKey_Quat (sTrack *track, float frame, float *out)
 {
  ...
  if (frame < 0.0) return 1;//clax_err_badframe;
  if (!track || !track->Keys) return 2;//return clax_err_nullptr;
