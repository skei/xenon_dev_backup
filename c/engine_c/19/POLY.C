...
//
 ...
 int left_inv_u,left_inv_v,left_inv_z;
 int left_inv_u_slope, left_inv_v_slope, left_inv_z_slope;
 int inner_inv_u_slope,inner_inv_v_slope,inner_inv_z_slope;
...
void CalcLeftTex(void)
 {
  ...
  float height;
  float invheight;
  float xslope,uslope,vslope,zslope;
  ...
  zslope = (v2->pPos.z - v1->pPos.z)*invheight;
  left_x_slope = float2fixed(xslope);
  left_inv_u_slope = float2fixed(uslope);
  left_inv_v_slope = float2fixed(vslope);
  left_inv_z_slope = float2fixed(vslope);
  //prestep = CEIL(v1->pPos.y) - v1->pPos.y;
  left_x = (1<<15) + float2fixed( v1->pPos.x + (v1->prestep * xslope));
  left_inv_u = (1<<15) + float2fixed( v1->u + (v1->prestep * uslope));
  left_inv_v = (1<<15) + float2fixed( v1->v + (v1->prestep * vslope));
  left_inv_z = (1<<15) + float2fixed( v1->pPos.z + (v1->prestep * zslope));
 }
...
void Poly_Tex( sPolygon *P )
 {
  ...
  P->v[0]->inv_z = 1 / P->v[0]->pPos.z;
  P->v[0]->inv_u = P->v[0]->u * P->v[0]->inv_z;
  P->v[0]->inv_u = P->v[0]->u * P->v[0]->inv_z;
 ...
 */
  ...
  inner_inv_u_slope = float2fixed((  (  (P->v[1]->inv_u  - P->v[0]->inv_u)
                                      * (P->v[2]->pPos.y - P->v[0]->pPos.y))
                                   - (  (P->v[2]->inv_u  - P->v[0]->inv_u)
                                      * (P->v[1]->pPos.y - P->v[0]->pPos.y)))
                                   * invdelta);
  inner_inv_v_slope = float2fixed((  ((P->v[1]->inv_v - P->v[0]->inv_v) * (P->v[2]->pPos.y - P->v[0]->pPos.y))
                                   - ((P->v[2]->inv_v - P->v[0]->inv_v) * (P->v[1]->pPos.y - P->v[0]->pPos.y)))
                                   * invdelta);
  // --- Z-clip poly ---
