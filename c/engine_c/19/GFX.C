...
int Load_GIF(const char* file, unsigned char *outpix, unsigned char * pal,
                               int* width, int* height)
 {
  ...
  prefix = (word*)malloc( sizeof(*prefix) * (MAX_CODES + 1) );
...
int Load_GFX(const char* file, unsigned char *outpix, unsigned char * pal,
                               int* width, int* height)
 {
  ...
  if (strstr(file, ".GIF"))
  {
   if (!Load_GIF(file,NULL,NULL,&w,&h)) return ERROR;
   outpix = (unsigned char *)malloc(w*h);
   Load_GIF(file,outpix,pal,NULL,NULL);
   return OK;
  }
  else
  if (strstr(file, ".PCX"))
  {
   if (!Load_PCX(file,NULL,NULL,&w,&h)) return ERROR;
   outpix = (unsigned char *)malloc(w*h);
   Load_PCX(file,outpix,pal,NULL,NULL);
   return OK;
  }
  else
  if (strstr(file, ".LBM"))
  {
   if (!Load_LBM(file,NULL,NULL,&w,&h)) return ERROR;
   outpix = (unsigned char *)malloc(w*h);
   Load_LBM(file,outpix,pal,NULL,NULL);
   return OK;
  }
  else
  if (strstr(file, ".TGA"))
  {
   if (!Load_TGA(file,NULL,NULL,&w,&h)) return ERROR;
   outpix = (unsigned char *)malloc(w*h);
   Load_TGA(file,outpix,pal,NULL,NULL);
   return OK;
  }
  return ERROR;
 }
