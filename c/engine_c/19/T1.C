...
void Engine_ProjectObject(sObject *o, sCamera *c)
 {
  ...
  // check if bounding sphere within view frustum.. (distance to planes)
...
void Engine_ApplyEnvMap(kfScene *scene)
 {
  sNode   *node;
  sObject *obj;
  int i;
  ...
  for (node = scene->World; node; node=node->Next)
  {
   if (node->Type==NodeTypeObject)
   {
    ...
    if ((obj->Flags & ObjFlagMapped)==0)
    {
     for (i=0;i<obj->NumVertices;i++)
     {
      obj->Vertices[i].u = obj->Vertices[i].Normal.x * (UVmult/2) + (UVmult/2);
      obj->Vertices[i].v = obj->Vertices[i].Normal.y * (UVmult/2) + (UVmult/2);
     }
     obj->Flags |= ObjFlagMapped;
    }
   }
  }
 }
...
void main(int argc, char *argv[])
 {
  ...
  if (argc != 3)
  {
   printf("T1 <3ds-file> <camera name>\n");
   exit(1);
  }
...
//  else printf("  - Couldn't find TEXTURE.PIX!\n");
  ...
  printf("þ Applying spherical mapping to unmapped objects\n");
  Engine_ApplyEnvMap(scene);

  printf("þ Setting up camera\n");
  // --- set active camera to "Camera01" ---
  node = Engine_FindByName (argv[2]);
  if (node) cam = (sCamera *)node->Object;
  else
  {
   printf("þ Camera named \"%s\" not found!\n",argv[2]);
   exit(1);
  }
  cam->SizeX = 320.0;
  cam->SizeY = 200.0;
