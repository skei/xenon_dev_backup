/*LOAD3DS.C*******************************************************************
   ...
   revision history :
     ...
     v0.10 (04/05/97) Borzom: Changed to fit clax structure.
   ...
   notes            :
     uhh.. used with permission, hi jare :)
...
static int add_key (t_KEY **track, t_KEY *key)
{
/*
...
*/
  ...
  switch (c_chunk_last) {
    ...
    case CHUNK_SHINSTRENGTH: rgb = &(mat->shin_strength); break;
    case CHUNK_TRANSPARENCY: rgb = &(mat->transparency); break;
    case CHUNK_TRANSFALLOFF: rgb = &(mat->trans_falloff); break;
    case CHUNK_REFBLUR:      rgb = &(mat->refblur); break;
    case CHUNK_SELFILLUM:    rgb = &(mat->self_illum);
  }
...
*/
  c_VERTEX *v = ((c_OBJECT *)c_node)->vert_orig;
  float     c[2];
  word      nv;
...
*/
  ...
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++)
      if (fread (&mat[i][j], sizeof (float), 1, f) != 1) return clax_err_badfile;
  if (fread (pivot, sizeof (pivot), 1, f) != 1) return clax_err_badfile;
  vec_make (pivot[0], pivot[1], pivot[2], &piv);
  vec_swap (&piv);
  mat_swap (mat);
//  if (mat_inverse (mat, mat) != clax_err_ok) return clax_err_undefined;
...
*/
  ...
  mat->texture.U_scale = 0.0;
  mat->texture.V_scale = 0.0;
  mat->texture.U_offset = 0.0;
  mat->texture.V_offset = 0.0;
  mat->texture.rot_angle = 0.0;
  mat->bump.file = NULL;
  mat->bump.U_scale = 0.0;
  mat->bump.V_scale = 0.0;
  mat->bump.U_offset = 0.0;
  mat->bump.V_offset = 0.0;
  mat->bump.rot_angle = 0.0;
  mat->reflection.file = NULL;
  mat->reflection.U_scale = 0.0;
  mat->reflection.V_scale = 0.0;
  mat->reflection.U_offset = 0.0;
  mat->reflection.V_offset = 0.0;
  mat->reflection.rot_angle = 0.0;
  c_node = mat;
...
*/
  ...
  if (c_chunk_last == CHUNK_TRACKCAMTGT) {
    cam->parent2 = parent;
    if ((track = malloc (sizeof (t_CAMERATGT))) == NULL) return clax_err_nomem;
  ...
  if (c_chunk_last == CHUNK_TRACKSPOTL) {
    light->parent1 = parent;
    if ((track = malloc (sizeof (t_SPOTLIGHT))) == NULL) return clax_err_nomem;
  ...
  if (c_chunk_last == CHUNK_TRACKLIGTGT) {
    ...
    if ((track = malloc (sizeof (t_LIGHTTGT))) == NULL) return clax_err_nomem;
...
static int read_TFLAGS (FILE *f, word *nf, t_KEY *key)
{
/*
  read_TFLAGS: Track flags/spline reader.
*/
  word  unknown, flags;
  int   i;
  float dat;
...
*/
  t_KEY *key, *track = NULL;
  float  pos[3];
  word   unknown[5];
  word   n, nf;

  if (fread (unknown, sizeof (word), 5, f) != 5) return clax_err_badfile;
  if (fread (&n, sizeof (n), 1, f) != 1) return clax_err_badfile;
  if (fread (unknown, sizeof (word), 1, f) != 1) return clax_err_badfile;
  while (n-- > 0) {
    if ((key = (t_KEY *)malloc (sizeof (t_KEY))) == NULL) return clax_err_nomem;
    if (read_TFLAGS (f, &nf, key)) return clax_err_badfile;
    if (fread (pos, sizeof (pos), 1, f) != 1) return clax_err_badfile;
    key->frame = nf;
...
*/
  ...
  c_chunk_last = parent;
  while ((pc = ftell (f)) < p) {
    if (fread (&h, sizeof (h), 1, f) != 1) return clax_err_badfile;
    c_chunk_curr = h.id;
    n = -1;
...
int clax_load_mesh_3DS (FILE *f)
{
/*
  clax_loadscene: loads mesh data from 3ds file "filename"
                  into scene "scene".
*/
  byte version;
  long length;
...
int clax_load_motion_3DS (FILE *f)
{
/*
