...
void Quat_Inv(float q[4], float qq[4])
{
 float l,norminv;
...
void Quat_To_Matrixt(float q[4], float mat[3][3])
{
...
void Matrix_to_Quat(float mat[3][3], float q[4])
{
 float tr,s;
 int i,j,k;
...
void AngAxis_to_Quat(float ang, float axis[3], float q[4])
{
 int i;
 float omega,s;
...
void Quat_To_AngAxis(float q[4], float *ang, float axis[3])
{
 int i;
 float omega,s,qn[4];
...
float RelAngAxis(float p[4], float q[4], float axis[3])
{
 float pinv[4],r[4],ang;

 Quat_Inv(p,pinv);
 Quat_Mult(pinv,q,r);
 Quat_To_AngAxis(r,&ang,axis);
 return(ang);
}
