...
//  if (mat_inverse (mat, mat) != OK) return ERROR;
  for (i = 0; i < obj->NumVertices; i++) {
    vec_sub (&v->Pos, &piv, &v->Pos);
    mat_mulvec (mat, &v->Pos, &v->Pos);
    v++;
  }
  return OK;
}
...
*/
  ...
  if ((light = (sLight *)malloc (sizeof (sLight))) == NULL)
    return ERROR;
  if (fread (c, sizeof (c), 1, f) != 1) return ERROR;
...
*/
  ...
  if ((cam = (sCamera *)malloc (sizeof (sCamera))) == NULL)
    return ERROR;
  if (fread (c, sizeof (c), 1, f) != 1) return ERROR;
...
*/
  ...
  if (f) {} /* to skip the warning */
  if ((mat = (sMaterial *)malloc (sizeof (sMaterial))) == NULL)
    return ERROR;
  mat->Id = c_id++;
  mat->Shading = 0;
  mat->Flags = 0;
...
*/
  ...
  if (strcmp (c_string, "$AMBIENT$") == 0) {
    if ((amb = (sAmbient *)malloc (sizeof (sAmbient))) == NULL)
      return ERROR;
  ...
  if (strcmp (c_string, "$$$DUMMY") == 0) {
    if ((obj = (sObject *)malloc (sizeof (sObject))) == NULL)
      return ERROR;
    obj->Id = 1024+c_id;
