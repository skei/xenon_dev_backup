...
void main(void)
{
 ...
 Object_Set_Rot(&torus,3072,2048,2048);
 Object_Set_Pos(&torus,500,100,1000);
 Object_Set_Mapping(&torus,0,texture);

 Object_Set_Rot(&dolph1,0,0,0);
 Object_Set_Pos(&dolph1,0,0,500);
 Object_Set_Mapping(&dolph1,0,texture);

 dolphin = Object_Copy(&dolph1);
 Object_Set_Rot(dolphin,3072,2048,2048);
 Object_Set_Pos(dolphin,300,50,-500);
 Object_Set_Mapping(dolphin,0,texture);

 Anim_Init(&dolph1,&dolph2,&dolph3,&dolphin_anim);

 Camera_Set_Rot(&cam,1,1,1);
 Camera_Set_Pos(&cam,1,1,1);
 cam.Bank = 0.0;
...
// Init camera path spline
 ...
 camera_path.p[0].x = -2000;
 camera_path.p[0].y = 2000;
...
// Dolphin 1
  ...
  if (object_visible(&dolph1,&cam) == 1)
  {
...
// Rotating torus
  ...
  Object_Add_Rot(&torus,10,15,5);
  if (object_visible(&torus,&cam) == 1)
  {
   Object_Project(&torus,&cam);
   Object_Draw(BackgroundBuffer,&torus);
   nv++;
  }

  for (i=0;i<nv;i++) BackgroundBuffer[i] = 255;
