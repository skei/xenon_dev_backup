...
// Read object name
 ...
 while ( !feof( fp ) )
 {
  ...
  if ( instr( buf, "Face " ) && !instr( buf, "list:" ) )
  {
   ...
   Polygon[ NumPolys ].b = a;
   Polygon[ NumPolys ].c = a;
...
1)
  ...
  Named object: "Torus1"
  Tri-mesh, Vertices: 128     Faces: 256
1 Mapped
  Vertex list:
2 Vertex 0:  X:-141.731018     Y:29.284071     Z:-296.967773     U:0.001118     V:-0.001727
...
1 Only if object is mapped
2 U/V if object is mapped
3 if material is assigned to object
4 if object is assigned to a smoothing group

ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ

char Header[4]; // "AOF" + ascii value for version #
float BSphereRadius;
int NumVertices;
{
 float x,y,z
...
int NumUV;
{
 float u,v;           // or 16.16 fixed point
}
int NumC;
{
 float c              // or 16.16 fixed point
}
int NumGroups
{
 int NumPolysInGroup
