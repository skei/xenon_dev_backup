...
// with a current_* and num_* for each, and a defined max_* to avoid overrun
 ...
 #define max_CENTERLIST 10000
 int current_CENTERLIST;
 CENTERLIST global_CENTERLIST[max_CENTERLIST];

 #define max_CORNER 20000
 int current_CORNER;
 CORNER global_CORNER[max_CORNER];

 #define max_CORNERLIST 10000
 int current_CORNERLIST;
 CORNERLIST global_CORNERLIST[max_CORNERLIST];

 #define max_EDGELIST 10000
 int current_EDGELIST;
 EDGELIST global_EDGELIST[max_EDGELIST];

 #define max_CUBES 10000
 int current_CUBES;
 CUBES global_CUBES[max_CUBES];
...
CORNER *setcorner (PROCESS *p, int i, int j, int k)
 {
  ...
  CORNER *c = &(global_CORNER[current_CORNER++]);
  if (current_CORNER >= max_CORNER) error("global_CORNER full");
  ...
  l = &(global_CORNERLIST[current_CORNERLIST++]);
  if (current_CORNERLIST >= max_CORNERLIST) error("global_CORNERLIST full");
...
int     setcenter(CENTERLIST *table[], int i, int j, int k)
 {
  ...
  new = &(global_CENTERLIST[current_CENTERLIST++]);
  if (current_CENTERLIST >= max_CENTERLIST) error ("global_CENTERLIST full\n");
...
void setedge (EDGELIST *table[], int i1, int j1, int k1,
                                int i2, int j2, int k2, int vid)
 {
  ...
  new = &(global_EDGELIST[current_EDGELIST++]);
  if (current_EDGELIST >= max_EDGELIST) error ("global_EDGELIST full\n");
...
triangle (int i1, int i2, int i3, VERTICES vertices)
 {
  gvertices = vertices;
  gntris++;
 #ifdef DEBUG_VERSION
...
void main (void)
 ...
 current_CENTERLIST = 0;
 current_CORNER = 0;
 current_CORNERLIST = 0;
 current_EDGELIST = 0;
 current_CUBES = 0;

 gntris = 0;
  //                    torus,sphere,blob                 //NOTET
  if ((err = polygonize(blob, .2, 10, 0.,0.,0., triangle, TET)) != NULL)
 ...
 printf("current_CENTERLIST = %i\n",current_CENTERLIST);
 printf("current_CORNER = %i\n",current_CORNER);
 printf("current_CORNERLIST = %i\n",current_CORNERLIST);
 printf("current_EDGELIST = %i\n",current_EDGELIST);
 printf("current_CUBES = %i\n",current_CUBES);
 printf("\nnumvertices: %i\n",gvertices.count);
