...
void CalcLeftShadow(void)
 {
  ...
  xslope = (v2->pPos.x - v1->pPos.x)/**invheight*//height;
...
void FillPolyShadow( int NumVert )
 {
  unsigned short int *ptr,*dptr;
  char color;
  int min_vtx;
  int vtx;
  int min_y,max_y;
  ...
  dptr = (unsigned short int *)destptr;
  ...
  dptr = &(Engine_Scene->ShadowBuffer[min_y*Engine_Scene->ScreenWidth]);          // buffer
...
void Poly_Shadow( sPolygon *P )
 {
  ...
  TopClip = 0;
  BotClip = ShadowSize-1;
  LeftClip = 0;
  RightClip = ShadowSize-1;

  ShadowValue = ((unsigned int)P) & 0xffff;
...
void CalcLeftGourTex(void)
 {
  ...
  left_u = (1<<15) + float2fixed( v1->u + (v1->prestep * uslope));
  left_v = (1<<15) + float2fixed( v1->v + (v1->prestep * vslope));
  // !!!! perhaps skip prestep yucku here???
