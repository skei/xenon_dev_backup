...
//
 ...
 int left_u,left_v,left_c,left_z;
 int left_u_slope, left_v_slope, left_z_slope;
 int inner_u_slope,inner_v_slope,inner_z_slope;
...
void CalcLeftTex(void)
 {
  ...
  left_z_slope = float2fixed(zslope);
  //prestep = CEIL(v1->pPos.y) - v1->pPos.y;
  left_x = (1<<15) + float2fixed( v1->pPos.x + (v1->prestep * xslope));
  left_z = (1<<15) + float2fixed( v1->pPos.z + (v1->prestep * xslope));
...
void FillPolyTex( int NumVert )
 {
  ...
  for(;;)
  {
   ...
   real_z1 = 1/left_z;
   real_u1 = left_u * real_z1;
   real_v1 = left_v * real_z1;
...
void Poly_Tex( sPolygon *P )
 {
  ...
  inner_v_slope = float2fixed((  ((P->v[1]->v - P->v[0]->v) * (P->v[2]->pPos.y - P->v[0]->pPos.y))
                               ...
                               * invdelta);
  inner_z_slope = float2fixed((  ((P->v[1]->pPos.z - P->v[0]->pPos.z) * (P->v[2]->pPos.y - P->v[0]->pPos.y))
                               - ((P->v[2]->pPos.z - P->v[0]->pPos.z) * (P->v[1]->pPos.y - P->v[0]->pPos.y)))
                               * invdelta);
