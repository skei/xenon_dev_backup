...
*/
  ...
  for (i=0 ; i<G->NumPolys ; i++)
  {
        ...
        && (v[2]->PosR.z != -1))
   {
    ...
    // Perhaps have to use fixedmul... ???
    poly->delta =   fixedmul(v[1]->PosR.x-v[0]->PosR.x,v[2]->PosR.y-v[0]->PosR.y)
                  - fixedmul(v[2]->PosR.x-v[0]->PosR.x,v[1]->PosR.y-v[0]->PosR.y);
    if ( poly->delta >= 0x8000)
 ...
 if (Object_CheckBoundingSphere( O, World->Camera) == 1)
 {
  ...
  // ------------------------------
   for (i=0;i<O->NumVertices;i++)
   {
    ...
    {
     O->Vertices[i].PosR.z = float2int(vector.z);
     vector.z += PROJ_VALUE;
     O->Vertices[i].PosR.x = float2fixed(((vector.x * PROJ_VALUE2)  / (vector.z)) + 160);
     n = float2fixed(((vector.y * PROJ_VALUE2)  / (vector.z)) + 100);
     O->Vertices[i].PosR.y = n;
     ps = 0x10000 - (n&0xffff);
     if (ps != 0x10000)
     {
      O->Vertices[i].ceil_y = (n & 0xffff0000) + 0x10000;
      O->Vertices[i].prestep = ps;
     }
     else
     {
      O->Vertices[i].ceil_y = (n & 0xffff0000);
      O->Vertices[i].prestep = 0;
     }
    }
   }
   // ------------------------------
