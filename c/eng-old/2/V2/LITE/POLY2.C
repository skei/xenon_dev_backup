...
void GTexScanLeft( sVertex *v1, sVertex *v2 )
{
 ...
 icslope  = (int)(cslope * 65536.0);
 c = (int)( (v1->c + (cslope * prestep) ) * 65536.0) + ((1<<16)-1);
...
//   TexInner(length,ptr,_Texture);
   for (j=0;j<length;j++)
   {
    *ptr++ = _ShadeTable[((c>>8)&0xff00) + _Texture[((v>>8)&0xff00)+((u>>16)&0xff)] ];
    u+=uadd;
    v+=vadd;
    c+=cadd;
   }
...
/*
 maxu = minu = v[0]->u;
 if (v[1]->u > maxu) maxu = v[1]->u; else if (v[1]->u < minu) minu = v[1]->u;
 if (v[2]->u > maxu) maxu = v[2]->u; else if (v[2]->u < minu) minu = v[2]->u;
 if ((maxu-minu) > (0.8*256))
 {
  d = ceil(maxu-minu);
  if (v[0]->u < (0.5*256)) v[0]->u += d;
  if (v[1]->u < (0.5*256)) v[1]->u += d;
  if (v[2]->u < (0.5*256)) v[2]->u += d;
 }
 maxv = minv = v[0]->v;
 if (v[1]->v > maxv) maxv = v[1]->v; else if (v[1]->v < minv) minv = v[1]->v;
 if (v[2]->v > maxv) maxv = v[2]->v; else if (v[2]->v < minv) minv = v[2]->v;
 if ((maxv-minv) > (0.8*256))
 {
  d = ceil(maxv-minv);
  if (v[0]->v < (0.5*256)) v[0]->v += d;
  if (v[1]->v < (0.5*256)) v[1]->v += d;
  if (v[2]->v < (0.5*256)) v[2]->v += d;
 }
*/
...
// c       b            c b
 ...
 cadd =  ( (v[1]->c - v[0]->c) * (v[2]->PosR.y - v[0]->PosR.y)
          -(v[2]->c - v[0]->c) * (v[1]->PosR.y - v[0]->PosR.y) )*divdelta;
 GTexFillPoly(_y1,_y2,(int)(uadd*65536.0),(int)(vadd*65536.0), (int)(cadd*65536.0));
}
