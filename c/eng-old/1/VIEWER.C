...
void main( int argc, const char *argv[] )
{
 ...
 if (Engine_Init(320,200,15,TRUE,10000,1) == FALSE)
 {
...
// Camera_SetPos(Camera,70,-315,-95);
// Camera_SetRot(Camera,1.3,0,-0.02);
...
// ------------------------------
 ...
 while (quit==0)
 {
  ...
  sprintf(tekst,"CAMERA POS %5.2f, %5.2f, %5.2f",Camera->Pos.x,Camera->Pos.y,Camera->Pos.z);
  drawstring(0,16,tekst,0x3fff,_VesaBuffer);
  sprintf(tekst,"CAMERA ROT %5.2f, %5.2f, %5.2f",Camera->Rot.x,Camera->Rot.y,Camera->Rot.z);
  drawstring(0,24,tekst,0x3fff,_VesaBuffer);
