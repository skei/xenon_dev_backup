 /*
  ...
  Debug this properly..
  Make  innerloop..
  Check if float/fixed/int versions of variables are correct, etc...

  - FastTex : No subpixel or anything... Just convert to int as soon as
    possible and use.. Like the good, old days.. :) Useful for EnvMap, etc.
  - Gouraud
  - Gouraud texture
  - Double gouraud texture (hazing + shading)
  - Bump (Env + bump
  - Alpha
  - Lambert
  - Lambert texture
...
static int CEIL(float f)
 ...
 //             00 = 24 bit, 10 = 53 bit, 11 = 64 bit (default)
 // bit 10,11 = Rounding
 //             00 nearest, or even (default)
 //             01 down (towards negative infinity)
 //             10 up
 //             11 chop towards zero

 volatile unsigned short int control_word;

void GetControlWord(void);
 #pragma aux GetControlWord=\
  " fstcw [control_word]";

void SetControlWord(void);
 #pragma aux SetControlWord=\
  " fldcw [control_word]";
