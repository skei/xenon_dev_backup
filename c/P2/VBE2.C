     ...
     if(VMI->BitsPerPixel == Depth)
     {
      if(VMI->NumberOfPlanes == 1)
      {
       if ((VMI->XResolution==X_Res) && (VMI->YResolution==Y_Res))
       {
        ...
        VGARegisterCompatible = 1;
        if ((VMI->ModeAttributes & 32) == 32) VGARegisterCompatible = 0;
...
void VesaUnInit(unsigned char *BitMap)
{
 Set_Vesa(3);
 UnmapPhysicalToLinear(BitMap);
}
